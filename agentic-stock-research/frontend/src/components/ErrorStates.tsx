...
const ERROR_CONFIGS = {
  ...
  server: {
    ...
    troubleshooting: [
      "The backend service may have crashed",
  ...
  timeout: {
    ...
    troubleshooting: [
      "Try analyzing fewer tickers at once",
  ...
  validation: {
    ...
    troubleshooting: [
      ...
      "Ensure maximum 5 tickers per request",
      "Verify country selection matches tickers",
      "Use valid date ranges for analysis"
    ]
  },
  generic: {
    icon: XCircleIcon,
  ...
  return (
    ...
    `}>
      <div className="text-center max-w-md">
        ...
        {(config as any).troubleshooting && (
          <details className="mt-4 text-left">
            <summary className="cursor-pointer text-sm font-medium text-gray-700 hover:text-gray-800">
              Troubleshooting Steps
            </summary>
            <ul className="mt-2 text-xs text-gray-600 space-y-1">
              {(config as any).troubleshooting.map((step: string, index: number) => (
                <li key={index} className="flex items-start">
                  <span className="inline-block w-4 h-4 text-center text-gray-400 text-xs leading-4 mr-2">
                    {index + 1}.
                  </span>
                  <span>{step}</span>
                </li>
...
export const TimeoutError: React.FC<Omit<ErrorStateProps, 'type'>> = (props) => (
  <ErrorState {...props} type="timeout" />
);
