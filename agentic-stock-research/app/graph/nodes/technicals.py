...
async def technicals_node(state: ResearchState, settings: AppSettings) -> ResearchState:
    # Guard: empty tickers causes IndexError that crashes the entire graph
    tickers = state.get("tickers") or []
    if not tickers:
        logger.warning("technicals_node: no tickers in state, skipping")
        state.setdefault("analysis", {})["technicals"] = {
            "labels": [], "closes": [], "indicators": {}, "signals": {}
        }
        state.setdefault("confidences", {})["technicals"] = 0.0
        return state
    ticker = tickers[0]
