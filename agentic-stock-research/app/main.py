...
def create_app() -> FastAPI:
    ...
    @maybe_observe()
    async def generate_pdf(request: Request, settings: AppSettings = Depends(get_settings)) -> Any:
        """
        Generate a PDF report.

        Accepts either:
          { "report_data": <full report object> }  ← FAST path (frontend sends existing data)
          { "tickers": ["TICKER"], "country": "India" }  ← SLOW path (re-runs full graph)

        Always prefer the fast path — the frontend should pass report_data so the
        backend can render the PDF without re-running the entire analysis pipeline.
        """
        try:
            ...
            if report_data:
                # ── Fast path: frontend passed the already-computed report ──────────
                # Validate it has the minimum fields needed for PDF rendering.
                if not isinstance(report_data, dict):
                    raise HTTPException(status_code=400, detail="report_data must be a JSON object")

                # Derive ticker robustly — check multiple possible locations
                ticker = (
                    report_data.get("ticker")
                    or report_data.get("decision", {}).get("ticker")
                    or "report"
                )
                logger.info("pdf_generation_started", tickers=[ticker], path="fast")

            else:
                # ── Slow path: re-run the analysis graph ─────────────────────────
                # This is a fallback only — the frontend should use report_data instead.
                tickers = body.get("tickers", [])
                if not tickers:
                    raise HTTPException(
                        status_code=400,
                        detail="Provide either 'report_data' (preferred) or 'tickers'"
                    )

                # Infer country from ticker suffix if not explicitly provided.
                # Without this, Indian stocks (.NS/.BO) were being re-analysed as
                # US stocks (country defaulted to "United States"), corrupting results.
                first_ticker = tickers[0] if tickers else ""
                inferred_country = "India" if (
                    first_ticker.upper().endswith(".NS") or
                    first_ticker.upper().endswith(".BO")
                ) else "United States"
                country = body.get("country") or inferred_country

                logger.info("pdf_generation_started", tickers=tickers, path="graph", country=country)

                # Map tickers to proper Yahoo Finance symbols (same as /analyze)
                mapped_tickers = []
                for _t in tickers:
                    try:
                        mapped_symbol, _ex, _cur = map_ticker_to_symbol(_t, country)
                        mapped_tickers.append(mapped_symbol)
                        logger.info(f"Mapped {_t} -> {mapped_symbol} [{_ex}] {_cur}")
                    except Exception as _e:
                        logger.warning(f"Failed to map ticker {_t}: {_e}")
                        mapped_tickers.append(_t)

                horizon_short = body.get("horizon_short_days")
                horizon_long  = body.get("horizon_long_days")

                graph = build_research_graph(settings)
                payload = {
                    "tickers":            mapped_tickers,
                    "horizon_short_days": horizon_short,
                    "horizon_long_days":  horizon_long,
                    "country":            country,
                    "analysis_type":      "institutional" if horizon_short and horizon_long else "standard",
                }
                state = await graph.ainvoke(payload)

                # Graph writes to state["final_output"]["reports"]
                ...
                ticker = report_data.get("ticker", mapped_tickers[0] if mapped_tickers else "report")

            # ── Render PDF from whichever path populated report_data ─────────────
            import json, pathlib
            pathlib.Path("/Users/varhag/workspace/equisense-ai/report_sample.json").write_text(json.dumps(report_data, default=str, indent=2))
