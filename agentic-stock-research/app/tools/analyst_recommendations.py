...
async def analyze_analyst_recommendations(ticker: str) -> Dict[str, Any]:
    ...
    def _fetch_analyst_data() -> Dict[str, Any]:
        try:
            ...
            # Try to get more detailed recommendation data
            try:
                ...
                if recommendations_df is not None and not recommendations_df.empty:
                    ...
                    recent_recs = recommendations_df.tail(10)  # Last 10 entries
                    for _, row in recent_recs.iterrows():
                        rec_data = {
                            "period": str(row.name) if hasattr(row, 'name') else "Recent",
                ...
                if upgrades_downgrades_df is not None and not upgrades_downgrades_df.empty:
                    ...
                    for _, row in recent_changes_df.iterrows():
                        change_data = {
                            "date": str(row.name) if hasattr(row, 'name') else "Recent",
